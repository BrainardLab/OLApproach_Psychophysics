function params = OLPsychophysics2IFCPulseConeNoiseStaircaseStimulus(exp, params)
% params = OLPsychophysics2IFCPulseConeNoiseStaircaseStimulus(exp)
...
diffPrimary1 = cacheModulation1.data(exp.observerAge).modulationPrimarySignedPositive-cacheModulation1.data(32).backgroundPrimary;
...
contrastSteps = 2:length(contrastLevels);
...
for T = 1:params.NTrialTypes
    for C = 1:params.NContrastLevels
        for v = 1:params.NVersionsPerStimulusType
            ...
            s(1, :) = 1;                % Background
            s(2, :) = contrastLevels(C)*params.StimulusModulationContrast(T);  % Modulation
            s(2, cosineWindowInIdx) = contrastLevels(C)*params.StimulusModulationContrast(T)*cosineWindowIn;
            s(2, cosineWindowOutIdx) = contrastLevels(C)*params.StimulusModulationContrast(T)*cosineWindowOut;
