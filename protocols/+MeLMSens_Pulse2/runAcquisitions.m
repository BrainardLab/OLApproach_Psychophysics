function acquisitions = runAcquisitions(acquisitions, oneLight,pSpot, trialResponseSys)
...
for block = acquisitions % loop over columns
    ...
    while ~abort && progressTotal < 1
        ...
        % Run trial
        [~, abort] = remainingAcquisitions(i).runNextTrial(oneLight, pSpot, trialResponseSys);   
...
function [progressTotal, progress, NTrialsRemaining] = getBlockProgress(block)
    ...
    for acquisition = block'
        NTrials = [NTrials acquisition.NTrialsTotal];
        NTrialsRemaining = [NTrialsRemaining acquisition.NTrialsRemaining];
    end
