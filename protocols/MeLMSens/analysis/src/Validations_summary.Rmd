...
luminancesBackgrounds = luminancesBackgrounds %>% filter(label != 'PreCorrection')
contrastsBackgrounds = contrastsBackgrounds %>% filter(label != 'PreCorrection')
contrastsFlicker = contrastsFlicker %>% filter(label != 'PreCorrection')
```

```{r luminancesBackgrounds, echo=FALSE}
  luminancesBackgrounds %>% 
  ggplot() +
    aes(x = direction, shape = label, y = lumActual) +
    geom_jitter() +
