...
parser.addParameter('plotCriterionLine',true);
parser.KeepUnmatched = true;
parser.parse(threshold, varargin{:});
ax = parser.Results.ax;
criterion = parser.Results.criterion;
plotCriterionLine = parser.Results.plotCriterionLine;
...
thresholdGroup.DisplayName = sprintf('Threshold = %.3f',threshold);

if ~isempty(criterion)
    if plotCriterionLine
...
% Place text marker
if ~isempty(criterion) && plotCriterionLine
