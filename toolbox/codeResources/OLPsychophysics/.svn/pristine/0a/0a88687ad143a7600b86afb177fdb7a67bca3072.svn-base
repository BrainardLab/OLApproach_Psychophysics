...
if exist(saveStimuliFile, 'file')
    load(saveStimuliFile);
else
    %% Event objects
...
for block = 1:params.NBlocks
    ...
    StimFirstOrSecond = StimFirstOrSecondOrdered%StimFirstOrSecond;%Shuffle(StimFirstOrSecondOrdered);
    
    for k = 1:params.NTrialsPerBlock
        system('say -r 210 Press key to start trial');
        pause;
